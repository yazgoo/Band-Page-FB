...
if (!function_exists('curl_init')) {
	...
	function curl_exec($ch)
	{
		...
		."--user-agent ".$ch->options[CURLOPT_USERAGENT]." ";
		foreach(explode("&", $ch->options[CURLOPT_POSTFIELDS]) as $p)
			$cmd .= " -F \"" . urldecode($p) ."\""; 
		foreach($ch->options[CURLOPT_HTTPHEADER] as $h)
			$cmd .= " -H \"" . $h . "\""; 
		$cmd .= " ". $ch->options[CURLOPT_URL]."";
		exec($cmd, $o);
		$result = implode(" ", $o);
		return $result;
	}
